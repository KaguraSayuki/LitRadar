...
`/admin/run/*` 检查访问会话或接口口令、同源条件与非回环保护。受限阶段要求 5 分钟
操作授权或兼容脚本的逐次管理员密码；取得流水线锁后调用统一次数校验再执行。CLI 和调度也在同一把锁内
检查次数，并使用相同的任务凭据快照。具体范围见 [部署与维护](deployment.md)。

`progress.py` 用 ContextVar 将阶段、处理数量和安全摘要传给当前任务的观察器。
`web/jobs.py` 在启动线程前获得流水线锁并校验次数，在线程内建立凭据上下文，完成后
释放锁；进度以独立短锁和原子文件替换持久化，不保存原始供应商错误或凭据。相同请求
标识返回已有任务，页面只轮询最新状态。确认运行锁空闲后的遗留任务标记为中断。
...
持久保存以跨 worker 限流，登录与操作解锁共用计数。阅读会话为密码哈希签名的 12 小时
HttpOnly Cookie；操作授权使用不同签名用途的 5 分钟 Cookie，换密码后两者均失效。
