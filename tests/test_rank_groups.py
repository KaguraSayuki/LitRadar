...
def test_同一篇在两个组里的分数可以不同(tmp_path, no_llm):
    ...
    cfg = _cfg(tmp_path, [
        {"slug": "org", "name": "有机",
         "keywords": {"core": ["carbene", "hybrid"]}},
        {"slug": "mat", "name": "材料", "keywords": {"core": ["perovskite"]}},
    ])
    conn = db.Database(cfg.db_file).connect()
    ids = db.sync_groups(conn, rank.load_groups(cfg))
    _seed(conn, "Carbene and perovskite hybrid", groups=[ids["org"], ids["mat"]])
    conn.commit()
    conn.close()
    ...
    got = _scores(cfg)
    org = got[("org", "Carbene and perovskite hybrid")]
    mat = got[("mat", "Carbene and perovskite hybrid")]
...
def test_只给本组成员打分(tmp_path, no_llm):
    ...
    cfg = _cfg(tmp_path, [
        {"slug": "org", "name": "有机", "keywords": {"core": ["carbene"]}},
        {"slug": "mat", "name": "材料", "keywords": {"core": ["perovskite"]}},
    ])
    conn = db.Database(cfg.db_file).connect()
    ids = db.sync_groups(conn, rank.load_groups(cfg))
    _seed(conn, "Carbene chemistry", groups=[ids["org"]])
    ...
    got = _scores(cfg)
    assert ("org", "Carbene chemistry") in got
    assert ("mat", "Carbene chemistry") not in got
...
def test_llm_rank_false_不调_LLM_但仍有分数(tmp_path, monkeypatch):
    cfg = _cfg(tmp_path, [
        {"slug": "free", "name": "不花钱的组", "keywords": {"core": ["carbene"]},
         "llm_rank": False},
    ])
    calls = []
    ...
    conn = db.Database(cfg.db_file).connect()
    ids = db.sync_groups(conn, rank.load_groups(cfg))
    _seed(conn, "Carbene chemistry", groups=[ids["free"]])
    conn.commit()
    conn.close()
    ...
    assert _scores(cfg)[("free", "Carbene chemistry")] is not None


def test_开着的组照常调_LLM(tmp_path, monkeypatch):
    cfg = _cfg(tmp_path, [
        {"slug": "paid", "name": "花钱的组", "keywords": {"core": ["carbene"]},
         "llm_rank": True},
    ])
    calls = []
    ...
    conn = db.Database(cfg.db_file).connect()
    ids = db.sync_groups(conn, rank.load_groups(cfg))
    _seed(conn, "Carbene chemistry", groups=[ids["paid"]])
    conn.commit()
    conn.close()
...
def test_一个组排序失败不影响其它组(tmp_path, monkeypatch, no_llm):
    cfg = _cfg(tmp_path, [
        {"slug": "bad", "name": "坏组", "keywords": {"core": ["carbene"]}},
        {"slug": "good", "name": "好组", "keywords": {"core": ["carbene"]}},
    ])
    conn = db.Database(cfg.db_file).connect()
    ids = db.sync_groups(conn, rank.load_groups(cfg))
    _seed(conn, "Carbene chemistry", groups=[ids["bad"], ids["good"]])
    conn.commit()
    conn.close()
    ...
    assert ("good", "Carbene chemistry") in _scores(cfg)
