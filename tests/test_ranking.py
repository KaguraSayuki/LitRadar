...
def test_反馈样本只把有分数的否决当负例(tmp_path):
    ...
    liked_id = _seed(conn, "Carbene insertion A")
    scored_id = _seed(conn, "Carbene insertion B")
    bare_id = _seed(conn, "Carbene insertion C")
    ...
    assert liked == ["Carbene insertion A"]
    assert disliked == ["Carbene insertion B"], "没分数的否决不该进负例"
...
def test_精排拿到反馈样本(tmp_path, monkeypatch):
    ...
    _seed(conn, "Metal carbene N-H insertion")
    star_id = _seed(conn, "Metal carbene asymmetric catalysis")
    drop_id = _seed(conn, "Metal carbene polymer coating")
    ...
    assert seen["disliked"] == ["Metal carbene polymer coating"]
    # 统计页据此判断闭环有没有在工作
...
def test_精排部分失败的条目不反超(tmp_path, monkeypatch):
    ...
    failed_id = _seed(conn, "Metal carbene chemistry metal carbene insertion")
    ok_id = _seed(conn, "Metal carbene overview")
...
def test_完全没有LLM分时仍然归一化(tmp_path, monkeypatch):
    ...
    cfg = _rank_cfg(tmp_path)
    conn = db.Database(cfg.db_file).connect()
    _seed(conn, "Metal carbene N-H insertion study")
    for filler in ("Polymer coating survey", "Total synthesis of a terpene",
                   "Surface analysis of thin films"):
        _seed(conn, filler)
    conn.commit()
