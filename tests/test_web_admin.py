"""Web write contracts: authorization, same-origin checks, locks and persistence."""
...
def test_legacy_access_token_rejects_invalid_values_and_accepts_header_or_cookie(client):
...
def test_guarded_stage_rejects_missing_or_wrong_password_and_accepts_correct_password(client, monkeypatch):
    pw = _set_password(monkeypatch)
    r = client.post("/admin/run/rank", headers={"X-Token": TOKEN})
    assert r.status_code == 401
    assert r.headers.get("X-Admin-Password-Required") == "1"
    assert client.post("/admin/run/rank", headers={
        "X-Token": TOKEN, "X-Admin-Password": "wrong-password"}).status_code == 401
    r = client.post("/admin/run/rank",
...
def test_daily_limit_allows_remaining_runs_then_blocks_at_limit(client):
    cfg = webapp.get_cfg()
    cfg.admin.cooldown_seconds = 0            # 隔离出每日上限这一条
    cfg.admin.daily_limit = 3
    midnight = datetime.now(ZoneInfo(cfg.app.timezone)).replace(
        hour=0, minute=0, second=5)
    for n in range(2):
        _log_run(cfg, "rank",
                 when=midnight.replace(second=5 + n).isoformat(timespec="seconds"))
    assert client.post("/admin/run/rank", headers={"X-Token": TOKEN}).status_code == 200
    _log_run(cfg, "rank", when=midnight.replace(second=7).isoformat(timespec="seconds"))
    r = client.post("/admin/run/rank", headers={"X-Token": TOKEN})
...
def test_昨天的运行不占今天的额度(client):
    ...
    yesterday = (datetime.now(ZoneInfo(cfg.app.timezone))
...
def test_pipeline_limit_counts_runs_per_stage_not_sum_of_stages(client, monkeypatch):
    """回归:每日上限曾经把 all 的 5 个子阶段记录**求和**,于是跑一次 all
    就变成 5 次,第二次直接被自己拦下。应按单个子阶段的最大次数算。"""
    cfg = webapp.get_cfg()
    monkeypatch.setattr(webapp.pipeline, "run_all", lambda *a, **kw: {})
    cfg.admin.cooldown_seconds = 0
    cfg.admin.daily_limit = 3
    midnight = datetime.now(ZoneInfo(cfg.app.timezone)).replace(
        hour=0, minute=0, second=5)
    ...
    for run in (1, 2):
        for n, name in enumerate(("ingest_mail", "ingest_search", "enrich", "rank", "summarize")):
            _log_run(cfg, name, when=midnight.replace(second=5 + run * 10 + n).isoformat(timespec="seconds"))
    response = client.post("/admin/run/all", headers={"X-Token": TOKEN})
