...
LEGACY = {
    ...
    "s2_queries": ['"N-H insertion" + aniline'],
    "keywords": {"core": ["Metal Carbene"], "bonus": ["enantioselective"]},
    "journals": {"core": ["Organic Letters"], "ok": ["Tetrahedron Letters"]},
    "seed_dois": ["10.1021/acs.orglett.7b01631"],
}
...
def test_老格式解析成_default_单组():
    ...
    # 字段照旧解析,不做任何搬运
    assert groups[0].core == ["metal carbene"]
    assert groups[0].queries == ['"N-H insertion" diazo aniline']
...
def test_groups_解析出多组与各自开关():
    groups = rank.load_groups(_cfg({"groups": [
        {"slug": "org", "name": "有机", "direction": "d1",
         "search_queries": ["a"], "keywords": {"core": ["carbene"]}},
